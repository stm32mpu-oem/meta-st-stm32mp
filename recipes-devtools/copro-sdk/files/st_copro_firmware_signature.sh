...
function verify_optee_key() {
    if [ -z "$SIGNATURE_KEY" ] ; then
        SIGNATURE_KEY=$TA_DEV_KIT_DIR/keys/default_rproc.pem
    fi
    # verify if key exist
...
# scripts/sign_rproc_fw.py  --in OpenAMP_TTY_echo_CM33_NonSecure.elf -out sign.bin --key keys/default_rproc.pem --plat-tlv NSBOOTADDR 0x80100000

# UC 2: signature / secure and non secure firmware
# scripts/sign_rproc_fw.py  --in OpenAMP_TTY_echo_CM33_NonSecure.elf --in tfm_s_ipcc.elf --out tfm_sign.bin --key keys/default_rproc.pem --plat-tlv SBOOTADDR 0x80000000  --plat-tlv NSBOOTADDR 0x80100000
...
if [ -n $INPUT_NSECURE ]; then
    ...
    $TA_DEV_KIT_DIR/scripts/sign_rproc_fw.py \
            --in $OUTPUT_NSECURE \
            --key $SIGNATURE_KEY \
            --plat-tlv NSBOOTADDR $DEFAULT_NSBOOTADDR \
            --out $filename
fi
# UC 2: signature / secure and non secure firmware
if [ -n "$INPUT_NSECURE" -a -n  "$INPUT_SECURE" ]; then
    ...
    $TA_DEV_KIT_DIR/scripts/sign_rproc_fw.py \
            ...
            --in $OUTPUT_SECURE  \
            --key $SIGNATURE_KEY \
            --plat-tlv SBOOTADDR $DEFAULT_SBOOTADDR \
            --out $filename
fi
