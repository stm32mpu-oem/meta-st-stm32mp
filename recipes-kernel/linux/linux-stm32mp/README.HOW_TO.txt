...
* Configure on a build directory (different of kernel source code directory)
    ...
    $ cd <directory to kernel source code>
    $> mkdir -p ../build
    $> make O="$PWD/../build" defconfig fragment*.config
    ...
    * manually one by one:
    ...
    ...
    $ (yes '' || true) | make oldconfig O="$PWD/../build"
    * or, by loop:
...
* Compile and install on a build directory (different of kernel source code directory)
    ...
    * Build kernel images (Image.gz and vmlinux) and device tree (dtbs)
    $> make Image.gz vmlinux dtbs LOADADDR=0xC2000040 O="$PWD/../build"
    * Build kernel module
    ...
    * Generate output build artifacts
    ...
    $> cp $PWD/../build/arch/$ARCH/boot/Image.gz $PWD/../build/install_artifact/boot/
    $> cp $PWD/../build/arch/$ARCH/boot/dts/st/st*.dtb $PWD/../build/install_artifact/boot/
    ...
    * Build kernel module
    $ make modules
    * Generate output build artifacts
    $ make INSTALL_MOD_PATH="$PWD/../build/install_artifact" modules_install
    $ mkdir -p $PWD/../build/install_artifact/boot/
    $ cp $PWD/../build/arch/$ARCH/boot/Image.gz $PWD/../build/install_artifact/boot/
    $ cp $PWD/../build/arch/$ARCH/boot/dts/st/st*.dtb $PWD/../build/install_artifact/boot/
...
Generated files are :
- $PWD/install_artifact/boot/Image.gz
